...
%% JASREP v4.0 — Methodology pivot (Session 19, 2026-04-21)
%%
%% v3.0 -> v4.0 changes driven by convergent critical cross-model review
%% (DeepSeek + Gemini, 2026-04-21; L1 §9 Stop Criterion #6 triggered):
%%   - Retitled to remove "calibration" (n=4 insufficient for that claim)
%%   - Abstract rewritten: shift from "we calibrate" to "we present preliminary
%%     measurement anchors + propose research program"
%%   - §1 Intro: added explicit disclaimer paragraph
%%   - §4.1 retitled "The Dwarapala measurement anchor"; §4.2 retitled "Literature
%%     comparison"; language tightened throughout
%%   - §5.1 retitled from "Significance of convergence" to "Site-level
%%     consistency within an order of magnitude"
%%   - §5.6 Limitations: strengthened monument-vs-settlement caveat
%%   - New §5.7 "A Research Program for Rigorous Testing" added (OSL,
%%     tephrochronology at non-monumental sites)
%%   - §6 Conclusions reframed: "preliminary" / "consistent with" / "require testing"
%%   - 51-pair dataset flagged as needing supplementary transparency (future work)
%% v3.0 was retained as companion reference; see submission_jasrep_v3.0.tex.
%% Target: ~18-22 pp single-identity geoarchaeology paper + research proposal.
%% Submission route: subscription (zero APC). Target: JASREP (Elsevier Q1).
%% Convergence analysis: external_reviews/CROSS_MODEL_CONVERGENCE_2026_04_21.md
...
\title{Cumulative Volcanic Burial and the Archaeological Detection Horizon in Java, Indonesia: Preliminary Measurement Anchors and a Research Program}
...
\begin{abstract}
Java hosts 45 active volcanoes in 129,000~km\textsuperscript{2}, and its archaeological record becomes progressively sparser with age in the volcanic interior. We ask a narrowly geomorphological question: at what rate has cumulative volcanic sedimentation buried the landscape, and what does that rate imply for the detection horizon of surface survey? We present preliminary measurement anchors and propose a research program to test them rigorously.

In 1803, the Dutch colonial administrator Nicolaus Engelhard documented two stone guardian statues at Candi Singosari in East Java---each 370~cm tall, carved from monolithic andesite in 1268~CE---with approximately half their bodies below ground. Gunung Kelud had erupted about twenty times in the intervening 535 years, and the landscape had risen around them at approximately 3.5~mm per year. The archival precision of this colonial-era observation requires primary-source verification, and we treat it here as a preliminary anchor rather than a validated calibration point. We extend the inference with three additional measurement anchors from the Merapi system in Central Java (Sambisari, Kedulan, Kimpulan, all 9th century~CE), obtaining site-level rates of 2.4--6.2~mm/yr across four sites in two volcanic systems and 350~km of Java. With $n=4$ and a shared methodological basis (depth divided by elapsed time since dated construction), these site-level rates fall within an order of magnitude but do not yet constitute a validated cross-regional calibration. We present them as preliminary measurement anchors requiring further geoarchaeological testing before they can be generalised.

Applied as a first-order linear projection at the site-level mean, these anchors place Kanjuruhan-era (\textasciitilde760~CE) deposits at 3.0--7.9~m and pre-400~CE deposits at 3.9--10.1~m depth in the Malang basin---depths that exceed conventional surface survey and approach or exceed the upper limit of ground-penetrating radar in volcanic sediment. A 51-pair eruption--site literature compilation yields a mean measured depth of 3.41~m over a 760-year horizon, consistent in order of magnitude with the site-level anchors (full dataset intended for supplementary release). Spatial analysis of 666 East Java sites shows near-volcano clustering that reflects survey intensity and monument durability rather than settlement distribution, and is offered as descriptive context not as a test of the taphonomic hypothesis.

The paper closes with a research program (Section 5.7) identifying specific techniques---OSL dating of soil profiles, tephrochronology in soil cores, targeted coring at non-monumental sites---by which these preliminary anchors could be upgraded to validated calibration. What we claim here is that existing evidence is consistent with a substantial Holocene detection horizon in volcanic Java's interior, sufficient to justify the proposed fieldwork. We do not claim this evidence yet establishes such a horizon.

\medskip
\noindent\textbf{Keywords:} volcanic taphonomy; cumulative sedimentation; detection horizon; East Java; preliminary measurement; research program; geoarchaeology
\end{abstract}
...
Hindu-Buddhist kingdoms flourished on Java from at least the 4th century~CE, yet the material record of the earliest Javanese polities is comparatively sparse. Stone temples of the 8th to 15th centuries survive because they are monumental; organic architecture does not. This paper does not attempt to reconstruct what the pre-monumental record looked like. Here the scope is narrower: to present preliminary measurements of the rate at which cumulative volcanic sedimentation has buried the Javanese landscape, and to evaluate what those measurements imply for the depth at which historical-era deposits now reside. We emphasise the word \emph{preliminary} from the outset: four measurement anchor sites in two volcanic systems are a starting point for a research program, not a validated cross-regional calibration.
...
The empirical foundation of this study is a set of four measurement anchors. We compiled construction dates and measured burial depths for four archaeological sites: the Dwarapala statues at Candi Singosari (\textasciitilde1268~CE, Kelud volcanic system) and three Hindu-Buddhist temples near Merapi in Central Java (Sambisari, Kedulan, and Kimpulan, all 9th century~CE). These span roughly 350~km of Java and two geologically distinct volcanic systems. Kelud has produced 37 confirmed eruptions since 1000~CE \citep{gvp2024}. Merapi is one of the world's most persistently active stratovolcanoes \citep{gertisser2012}. The site-level rates fall within a single order of magnitude (2.4--6.2~mm/yr), consistent across both volcanic systems---but with $n=4$ and shared methodological basis, we treat this as preliminary agreement rather than validated convergence. A rigorous Java-wide calibration requires the further testing outlined in Sect.~5.7.

Those rates are applied to project burial depths for successive historical periods: Kanjuruhan-era (\textasciitilde760~CE) remains in the Malang basin are predicted (at the anchor-site mean rate) to lie at 3.0--7.9~m---depths that exceed the reach of surface survey and approach or exceed the upper limit of ground-penetrating radar in volcanic sediment \citep{putra2019,conyers2004}. A spatial analysis of 666 known archaeological sites in East Java is included for descriptive context rather than as a test of the anchor-site observations: the distribution is shaped by both survivorship (monumental stone versus organic architecture) and two centuries of survey concentrated on the Singosari--Majapahit heartland, and cannot by itself confirm or refute a taphonomic interpretation.
...
\begin{table}[t]
\caption{Projected burial depth by era using the preliminary anchor-site rates. \textbf{Note: depths are upper bounds (uncompacted projections).} Unconsolidated volcanic sediment compacts by 10--30\% at depths exceeding approximately 5~m (Sect.~5.6); realised depths for Kanjuruhan-era and earlier rows are therefore likely 10--30\% shallower than shown. The projections assume basin or plain settings with net aggradation; ridge and slope contexts may experience net erosion (Sect.~5.6).}
\label{tab:depth}
\begin{tabular}{lcccccc}
\toprule
...
Four measurement anchors across two volcanic systems and 350~km of separation yield preliminary site-level cumulative aggradation rates that fall within a single order of magnitude (2.4--6.2~mm/yr). With $n=4$ and a shared methodological basis (burial depth divided by elapsed time since dated construction), we explicitly do \emph{not} claim that this constitutes a validated Java-wide calibration; we claim only that the four sites are consistent with a substantial cumulative-burial regime and that the evidence is sufficient to motivate the research program in Section~5.7. The Dwarapala anchor in particular depends on a qualitative colonial-era observation whose primary-source verification remains outstanding.

The 51-pair eruption--site literature compilation, while order-of-magnitude consistent with the anchor-site rates (mean 3.41~m over a 760-year horizon versus 3.3~m predicted), shares selection biases with the four-site sample and is best treated as a comparison rather than a formally independent validation.

If the preliminary rates are approximately correct, their implications for archaeological detection are material. Kanjuruhan-era remains (\textasciitilde760~CE) in the Malang basin would lie at 3.0--7.9~m depth; pre-400~CE remains at 3.9--10.1~m. Both ranges exceed conventional surface survey capability and approach or exceed the upper limit of ground-penetrating radar in volcanic sediment. These projections do not prove that settlement remains exist at these depths in any particular location, and they rest on a rate estimate that itself requires the further testing proposed in Section~5.7. What they do establish is a hypothesis sufficiently well-motivated to warrant rigorous geoarchaeological testing rather than continued assumption either way.

The distribution of known sites provides context but cannot test the framework. The 666-site dataset is composed of monuments that survived burial and that were found by surveys concentrated on the Singosari--Majapahit heartland; any interpretation of the distribution must account for both filters, and the strong spatial autocorrelation (Moran's $I = 0.937$) in volcanic distance limits what regression-based inference can establish. What can be said without reservation is that the apparent archaeological invisibility of pre-monumental Javanese settlement is \emph{consistent with} a geomorphologically plausible rate of cumulative burial operating across the historically active volcanic terrain---not that it has been demonstrated to arise from that burial. The paper's contribution, narrowly, is a set of preliminary measurement anchors and a concrete research programme for testing them.
