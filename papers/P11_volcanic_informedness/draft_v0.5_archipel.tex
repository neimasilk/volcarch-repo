...
The island hosts 34 of Indonesia's 127 active volcanoes, erupting on average once every 1--2 years.\footnote{Global Volcanism Program, \emph{Volcanoes of the World}, v.~5.1.1 (Smithsonian Institution, 2023), \url{https://volcano.si.edu/}.}
...
Of the 142 candi for which coordinates were compiled from published surveys, heritage registries, and standard catalogues,\footnote{Jacques Dumar\c{c}ay, \emph{The Temples of Java} (Oxford: Oxford University Press, 1993). R.\ Soekmono, \emph{The Javanese Candi: Function and Meaning} (Leiden: Brill, 1995).} all were either continuously visible or rediscovered during colonial-era earthworks.
...
The mean bearing is 279\textdegree{} (west), with nearly half of all candi (47.2\%) on western flanks; the eastern quadrant contains fewer than 4\%.\footnote{Rayleigh test for circular non-uniformity: $p = 3.4 \times 10^{-8}$.}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig1_candi_polar_bearings.png}
\caption{Polar plot of candi bearings from nearest volcanic peak. The western clustering (mean 279\textdegree{}) is consistent with positioning on the leeward side of prevailing eruption plumes.}
\label{fig:polar}
\end{figure}

This western concentration has both practical and cosmological dimensions.
Prevailing winds in Java blow from west to east during the eruption-prone dry season, depositing tephra preferentially on eastern flanks.
Western slopes thus receive less volcanic fallout---a practical advantage for settlements that the builders may well have understood from accumulated experience.
...
Zone A (within 10~km of a volcanic peak) contains 42.3\% of all candi---an overrepresentation of 17.9$\times$ relative to land area.
...
We georeferenced 170 inscriptions from the DHARMA epigraphic database,\footnote{DHARMA project, \emph{The Domestication of ``Hindu'' Asceticism and the Religious Making of South and Southeast Asia: Epigraphic Database} (CNRS/ERC, 2024), \url{https://dharma.hypotheses.org/}.} producing a spatially resolved inscription dataset for the region.

The contrast is stark.
Inscriptions are on average 9.2~km farther from volcanoes than candi.\footnote{Mann-Whitney $U$ test: $p = 5.2 \times 10^{-8}$.}
While 42.3\% of candi lie in Zone A (within 10~km of a volcano), only 12.9\% of inscriptions do.
...
Before 929~CE, 57\% of inscriptions are located in the court zone, and 91\% are Sanskrit-dominant.
After 929, 53\% of inscriptions appear in the periphery ($>$30~km from volcanoes), and 89\% of those are mixed or indigenous-rich in vocabulary.
...
Located 5.5~km from Mount Sindoro on its western flank---Zone A, western quadrant---the site was found not by archaeologists but by sand miners, who exposed a complete Mataram-era settlement beneath 6--8~m of pyroclastic deposits.\footnote{Novida Abbas, ed., \emph{Liangan: Mozaik Peradaban Mataram Kuno di Lereng Sindoro} (Yogyakarta: Kepel Press / Balai Arkeologi Yogyakarta, 2016).}
...
The most productive survey depths---3 to 9~m, based on colonial-era burial data and the Liangan precedent---are within the range of electrical resistivity tomography and ground-penetrating radar.
...
We tested this using adversarial regression controlling for three survey-intensity proxies (road distance, proximity to heritage offices, university proximity).
After controlling for all three, volcanic proximity still explains additional variance in site counts.\footnote{Quasi-Poisson GLM: volcanic proximity $\beta = -0.477$, $p = 0.0015$.}
...
Crucially, the model generates testable predictions: ground-penetrating radar survey of ten western-flank sites at 3--7~m depth should yield buried settlement remains.
