...
| **K4** | *(our own proposed explanation)* the TGB null is "predicted", because `road_dist` is not a model feature so TGB has nothing to cancel | ⛔ **TESTED AND FAILED — E224, 2026-08-03.** Adding `road_dist` to the feature set changes nothing: TGB − random = −0.022 with it, −0.025 without, 30% of pairs positive in both. The null is **unexplained**; say so. Caveat: `road_dist` correlates +0.49 with `river_dist`, so the manipulation was not clean — filed FAILED, not REFUTED. |
