...
| burial_depth_m | float | Burial depth in meters (0 = found at surface) |
...
- Positive burial depths (n=32): range 0.60-9.14 m, mean 2.88 m, median 1.75 m (measured subset only, n=27: median 2.00 m)
...
Data extraction from OCR'd OV volumes was AI-assisted (large language model). Each record cites its source volume and page (`ov_page`) so it can be checked against the original text. See `REGISTER_NOTES.md` for limitations (OCR quality, approximate coordinates).
