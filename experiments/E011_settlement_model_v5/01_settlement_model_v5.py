...
Builds on E010 (AUC=0.711). Sweeps TGB sampling parameters using fixed spatial
CV fold assignment, then runs full evaluation on the best configuration.

Run from repo root:
    py experiments/E011_settlement_model_v5/01_settlement_model_v5.py
"""
...
def main():
    ...
    print("=" * 60)
    print("E011: Settlement Suitability Model v5 (TGB parameter sweep)")
    print("=" * 60)
    ...
    if not road_dist_path.exists():
        print(f"ERROR: Missing {road_dist_path}. Run: py tools/compute_road_distance.py")
        sys.exit(1)
    ...
    print(f"  road_dist proxy: range [{np.nanmin(road_arr):.1f}, {np.nanmax(road_arr):.1f}]")
    ...
    results_txt = f"""E011 - Settlement Suitability Model v5 (TGB sweep) Results
===============================================
Date: 2026-02-24
Feature set: {FEAT_COLS}
Pseudo-absence strategy: Target-Group Background (road-distance weighted)
...
AUC progression:
  ...
  E009: 0.664
  E010: 0.711
  E011(best): {best_metrics['xgb_mean_auc']:.3f}
...
"""
    ...
    fig.suptitle(
        f"E011 TGB Sweep - Best config AUC={best_metrics['xgb_mean_auc']:.3f} ({verdict})",
        fontsize=11,
    )
    ...
    print("\n" + "=" * 60)
    print(f"E011 complete. Best AUC={best_auc:.3f} | {verdict}")
    print("=" * 60)
