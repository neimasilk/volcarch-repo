% Asian Perspectives — Submission v0.1
% Target: Asian Perspectives (University of Hawai'i Press, Q1, $0 APC)
% Style: Chicago 16th ed. author-date
% Submit as Word/RTF via http://asianperspectives.msubmit.net
% Convert: pandoc submission_asianperspectives_v0.1.tex -o submission_asianperspectives_v0.1.docx
...
\author{
  Mukhlis Amien\textsuperscript{1,*}\\[4pt]
  ...
  \small ORCID: https://orcid.org/0000-0000-0000-0000 % TODO: Replace with actual ORCID
}
...
\centerline{\small (b)}
\end{minipage}
\caption{Direct photographic evidence of cumulative volcanic sedimentation at Candi Singosari. (a)~Colonial-era photograph (c.~1860) from the Leiden University Library archives, captioned \textit{Hindoe Oudheden---Singosari}, showing the Dwarapala guardian statue partially buried beneath volcanic sediment approximately six centuries after its construction (A.D.~\textasciitilde 1268). Only the head, shoulders, and hands remain visible above the accumulated overburden. (b)~The same statue today, fully excavated and displayed at its original seated height of 370~cm. At the calibrated sedimentation rate of 3.5~mm/yr derived from this site (Section~3.2), approximately 185~cm of volcanic overburden accumulated between construction and the statue's rediscovery in A.D.~1803.}
...
Our compilation of known archaeological sites in East Java (Section~3.1) identified 666 unique entries, of which only 391 (59\%) have usable spatial coordinates. The dataset is dominated by stone monuments (\textit{candi}) from the Singosari and Majapahit periods (13th--15th centuries A.D.) \citep{kinney2003}. Earlier periods are poorly represented: pre-10th century sites constitute less than 5\% of the geocoded total.
...
\textbf{Limitation:} The dataset is heavily biased toward stone monuments (\textit{candi}) that survived volcanic burial due to their monumental scale. Wooden settlements, which likely constituted the vast majority of historical habitation, are systematically absent---precisely the pattern predicted by the taphonomic bias hypothesis. We note that Indonesia currently lacks a comprehensive, centralized digital archaeological site registry; the dataset assembled here represents the most complete compilation achievable from publicly available sources, but systematic undercounting is likely.
...
\begin{itemize}
  \item Construction date: A.D.~\textasciitilde 1268 (reign of Kertanegara, Singosari Kingdom)
  \item Discovery date: A.D.~1803, by Nicolaus Engelhard
...
\caption{Dwarapala Singosari burial timeline. The statues were constructed A.D.~\textasciitilde 1268 at full height (370~cm) and discovered in A.D.~1803 with approximately half (185~cm) buried by volcanic sedimentation, yielding a cumulative rate of 3.5~mm/yr. The cross-system mean from four calibration points is $4.4 \pm 1.2$~mm/yr.}
\label{fig:timeline}
\end{figure}
...
\toprule
Site & Built (A.D.) & Discovered & Depth (cm) & System & Rate (mm/yr) & Source \\
\midrule
...
\textbf{E005---Terrain-controlled analysis.} To separate the effect of terrain suitability from volcanic proximity, we constructed a terrain suitability index combining four DEM-derived features: slope ($<$15\textdegree{} preferred), elevation (100--800~m a.s.l.), topographic wetness index (TWI $>$ median), and river proximity ($<$5~km). Each feature was scored 0--1 and equally weighted. The study area was divided into a 25~km $\times$ 25~km grid (187 cells). For each cell, we computed the residual between observed and terrain-predicted site count. Spearman correlation between residual density and distance to nearest volcano tests the taphonomic bias hypothesis after controlling for terrain preference.
...
The Dwarapala sedimentation calculation yields a rate of 3.5~mm/year (Section~3.2). This rate is internally consistent with documented Kelud eruption histories, which account for approximately 100 of the 185~cm total burial through direct tephra deposition alone. The remaining 85~cm is attributable to secondary remobilization and contributions from other volcanic systems.

The secondary calibration points yield rates of 2.4--6.2~mm/yr across the Merapi system, consistently higher than the Dwarapala rate (3.5~mm/yr) from the Kelud system. This difference is physically plausible: Merapi erupts more frequently than Kelud, and the Central Java sites are closer to their volcanic source. The cross-system mean of $4.4 \pm 1.2$~mm/yr suggests that ongoing volcanic burial is a Java-wide phenomenon with quantifiable, consistent rates.
...
\begin{tabular}{lcccccc}
\toprule
Era & Date (A.D.) & Elapsed (yr) & Low (2.4) & Dwarapala (3.5) & Mean (4.4) & High (6.2) \\
\midrule
...
Preliminary independent validation from geotechnical borehole literature supports these rates. A screening of published geotechnical reports identifies six additional buried archaeological sites not included in our calibration set: Candi Liangan (3.6~mm/yr; Merapi-Sindoro system, a non-temple settlement), Candi Pendem (3.3~mm/yr), Candi Morangan (3.2~mm/yr), Candi Kadisoka (2.7~mm/yr), Candi Ijo (1.4~mm/yr), and Pleret palace complex (2.3~mm/yr; a non-temple palatial site). The mean rate across these six independent sites is 2.8~mm/yr, converging with the Dwarapala rate (3.5~mm/yr) and falling within the calibration range reported here. Notably, Liangan represents a third volcanic system (Sindoro) and a non-monumental site type, while Pleret is a post-Majapahit palatial complex---both extending the applicability of the framework beyond temple architecture and beyond the Kelud-Merapi pair. A full analysis incorporating these and additional sites from the geotechnical literature is in preparation.
...
While the taphonomic bias hypothesis cannot be tested with surface distribution data alone (Section~4.3), it is falsifiable through subsurface investigation. Specifically, the hypothesis predicts that systematic subsurface survey in high-suitability volcanic zones (Zone~B in our terrain model) should reveal buried archaeological features at the projected depths. If a comprehensive subsurface survey program---employing deep GPR, borehole coring, or systematic excavation---were conducted across multiple high-suitability locations in the Malang basin and found \textit{no} buried remains at the predicted depths, this would constitute strong evidence against the hypothesis. The absence of buried sites in zones where the model predicts their presence would suggest that the observed site distribution genuinely reflects past settlement patterns rather than differential preservation.
...
As a thought experiment: at 4.4~mm/yr, a Kutai-era (A.D.~\textasciitilde 400) artifact in the Malang basin would now lie beneath \textasciitilde7~m of overburden. The same artifact in Kalimantan (zero volcanism) sits at the original ground surface. We do not claim that Javanese polities necessarily predate Kutai, only that the differential preservation conditions between volcanic and non-volcanic islands must be considered before interpreting surface-based chronologies as reflecting genuine temporal precedence \citep{coedes1968}.
...
\begin{enumerate}
  ...
  \item \textbf{Rate constancy:} We treat sedimentation as temporally uniform, smoothing over episodic large eruptions. The Dwarapala rate is a cumulative average over \textasciitilde535 years.
  ...
  \item \textbf{Site type bias:} All four calibration sites are monumental stone temples. Non-monumental sites (wooden settlements, earthworks) may experience different effective burial rates due to differences in surface expression, compaction, and post-depositional disturbance. Preliminary evidence from Liangan (a non-temple settlement) and Pleret (a palatial complex) suggests comparable rates, but systematic validation across site types remains a priority.
...
Three follow-up studies are planned: (1)~a machine learning settlement suitability model independently tested against volcanic burial zones (Paper~2; an interactive dashboard exploring model predictions is available at \url{https://volcarch-dashboard.streamlit.app}); (2)~a spatially-resolved burial depth model calibrated against all four anchor points (Paper~3); and (3)~targeted GPR survey at 5--10 locations in high-suitability, high-burial-depth zones to test for subsurface anomalies.

% ============================================================
\section{Conclusions}

We have presented a quantitative framework for estimating volcanic taphonomic bias in the Indonesian archaeological record. Four empirical calibration points from two independent volcanic systems yield cumulative sedimentation rates of 2.4--6.2~mm/yr (mean $4.4 \pm 1.2$~mm/yr), suggesting that multi-meter burial is a systematic, Java-wide phenomenon.
...
Our projections indicate that Kanjuruhan-era (A.D.~\textasciitilde 760) remains in the Malang basin lie beneath 3.0--7.9~m of overburden, and pre-Hindu (A.D.~\textasciitilde 400) remains beneath 3.9--10.1~m. These depths exceed conventional survey capabilities and approach the limits of ground-penetrating radar. Future investigation in volcanic Java should incorporate subsurface detection methods in zones identified as having both high settlement suitability and high expected burial depth.

% ============================================================
\section*{Data Availability}

The archaeological site dataset, DEM derivatives, and analysis scripts are available at \url{https://github.com/neimasilk/volcarch-repo}. Raw DEM data from Copernicus GLO-30 are publicly available at \url{https://dataspace.copernicus.eu/}. An interactive dashboard for exploring settlement suitability model predictions is available at \url{https://volcarch-dashboard.streamlit.app}. All analysis scripts (E001--E006) are available in the project repository under \texttt{experiments/}.

\section*{Acknowledgments}

The author thanks the Global Volcanism Program (Smithsonian Institution) for open eruption data, and the OpenStreetMap and Wikidata communities for structured geospatial data. AI-assisted tools (Claude, Anthropic) were used as a writing companion during manuscript preparation for code development assistance and data analysis scripting; the AI tools did not contribute to research design, hypothesis formulation, or scientific interpretation, and all results were independently verified by the author. This research received no specific grant from any public, commercial, or not-for-profit sector.
