...
Robustness checks across 20 alternate random seeds confirmed stable performance (range: 0.729--0.774; all seeds exceed the null-model ceiling).
...
Challenge 1 (tautology test) showed no disqualifying tautology signal across all runs (negative Spearman $\rho$ between suitability and volcano distance), with temporal split validation providing the strongest anti-tautology evidence (AUC = 0.755 on held-out hard-access sites).
...
Our approach aligns with this trajectory while explicitly isolating pseudo-absence bias as a first-order problem \citep{barbet2012,senay2013,wisz2009,lobo2010}. Foundational work on presence-only modeling \citep{phillips2006,kvamme2006} and spatial prediction in heterogeneous landscapes \citep{nuninger2016,araujo2005} informs our experimental design.
...
Tephra thickness estimation via the exponential thinning model \citep{pyle1989} provides a quantitative link between eruption magnitude and burial potential. For archaeology, volcanic tephra burial has been documented in Indonesia \citep{degroot2009,bourdier1997}, Europe \citep{french2003}, and Oceania \citep{torrence2002,grattan2007}. The Smithsonian Global Volcanism Program catalogue \citep{siebert2010} provides the eruption chronology underlying burial depth estimation.
...
To complement the gain-based feature importance (Section~3.3), we computed TreeSHAP values \citep{lundberg2017} for the best E013 XGBoost model (Figure~\ref{fig:shap}). TreeSHAP provides exact, instance-level feature attributions grounded in cooperative game theory, avoiding the approximation errors of permutation-based approaches.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig13_shap_beeswarm.png}
...
This supports the view that data-generation assumptions dominate model behavior under biased observation regimes \citep{georganos2021}.
...
\textbf{Archaeological site data:} The compiled East Java archaeological site dataset (666 sites) is available in the repository at \texttt{data/processed/east_java_sites.geojson}. Sources include OpenStreetMap (ODbL license), Wikidata (CC BY-SA), Wikipedia Indonesia, and BPCB Jawa Timur publications.

\textbf{Environmental covariates:} The Copernicus GLO-30 DEM and derived layers (slope, aspect, TWI, TRI, river distance, road distance) are available from the Copernicus Programme (ESA) at https://dataspace.copernicus.eu/ (free and open license). OpenStreetMap road and river data are available from openstreetmap.org (ODbL license).

\textbf{Code and software:} All source code, including the E013 hybrid model implementation, temporal validation (E014), and robustness checks, is available at GitHub: https://github.com/neimasilk/volcarch-repo (commit \texttt{453f36b}, MIT license).

\textbf{Supplementary outputs:} Seed stability analysis, block-size sensitivity results, and figure generation scripts are available in \texttt{papers/P2\_settlement\_model/supplement/} and \texttt{papers/P2\_settlement\_model/figures/}.

\textbf{Dependency lock:} Full Python package versions are documented in \texttt{papers/P2\_settlement\_model/requirements\_submission\_lock.txt}.
...
The complete codebase for reproducing all experiments (E007--E014), figures, and analysis is available at:\\ \url{https://github.com/neimasilk/volcarch-repo}

\textbf{Key scripts:}
\begin{itemize}
  \item \texttt{experiments/E013\_settlement\_model\_v7/01\_settlement\_model\_v7.py} — E013 hybrid model implementation
  \item \texttt{experiments/E014\_temporal\_validation/01\_temporal\_split\_test.py} — Temporal validation (E014)
  \item \texttt{papers/P2\_settlement\_model/robustness\_checks.py} — Seed stability analysis
  \item \texttt{papers/P2\_settlement\_model/block\_size\_sensitivity.py} — CV block size testing
  \item \texttt{papers/P2\_settlement\_model/build\_figures.py} — Figure generation pipeline
\end{itemize}

\textbf{Software:} Python 3.11+, geopandas 0.14.0, rasterio 1.3.8, xgboost 2.0.0, scikit-learn 1.3.0. Full dependencies in \texttt{requirements\_submission\_lock.txt}.
