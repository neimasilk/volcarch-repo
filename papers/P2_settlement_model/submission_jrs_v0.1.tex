% Journal of Remote Sensing (SPJ/AAAS) — Submission v0.1
% Adapted from MDPI Remote Sensing v0.3
...
%%%%%% Authors %%%%%%
\author[1*]{Mukhlis Amien}

%%%%%% Affiliations %%%%%%
\affil[1]{Lab Data Sains, Universitas Bhinneka Nusantara, Indonesia.}
\affil[*]{Address correspondence to: amien@ubhinus.ac.id}
...
Presence-background models are sensitive to sample selection bias, especially when background points do not reflect the same observation process as presences \cite{phillips2009,elith2006,araujo2006}.
For spatially clustered data, random train/test splits overestimate performance \cite{kohavi1995}.
...
Archaeological predictive modeling has long recognized that discovery processes can dominate apparent settlement patterns \cite{verhagen2012,kamermans2019,verhagen2020}.
...
Our approach aligns with this trajectory while explicitly isolating pseudo-absence bias as a first-order problem \cite{barbet2012,senay2013,wisz2009,lobo2010}. Foundational work on presence-only modeling \cite{phillips2006,kvamme2006} and spatial prediction in heterogeneous landscapes \cite{nuninger2016,araujo2005} informs our experimental design.
...
Tephra thickness estimation via the exponential thinning model \cite{pyle1989} provides a quantitative link between eruption magnitude and burial potential. For archaeology, volcanic tephra burial has been documented in Indonesia \cite{degroot2009,bourdier1997}, Europe \cite{french2003}, and Oceania \cite{torrence2002,grattan2007}. The Smithsonian Global Volcanism Program catalogue \cite{siebert2010} provides the eruption chronology underlying burial depth estimation.
Taphonomic bias from volcanic burial represents a systematic visibility constraint that can be misinterpreted as settlement absence \cite{wandsnider1992}.
...
Primary metrics are AUC and TSS \cite{allouche2006}; TSS is preferred over kappa for presence-background models as it is independent of prevalence \cite{jimenez2020}.
...
Table~\ref{tab:progression} summarizes the full experiment progression.
...
Across 20 alternate random seeds, E013 yielded a mean XGBoost AUC of 0.751 (95\% bootstrap CI: 0.745--0.756; range: 0.729--0.774; 11 of 20 seeds $\geq$ 0.75). While the mean is near-threshold, even the worst seed (0.729) exceeds the DKNS null-model ceiling (0.646) by $+$0.083, confirming that environmental signal is robust across all seed realizations.
...
Top XGBoost feature importances (gain-based; see also SHAP-based analysis \cite{lundberg2017} in Section~\ref{sec:shap}): elevation (0.215), TRI (0.185), TWI (0.166), river distance (0.160), slope (0.155), aspect (0.118).
...
To complement the gain-based feature importance (Section~3.3), we computed TreeSHAP values \cite{lundberg2017} for the best E013 XGBoost model (Figure~\ref{fig:shap}). TreeSHAP provides exact, instance-level feature attributions grounded in cooperative game theory, avoiding the approximation errors of permutation-based approaches.
...
\caption{SHAP beeswarm plot for the best E013 XGBoost model, showing instance-level feature contributions. Each dot represents one training sample; color indicates feature value (red = high, blue = low). Features are ordered by mean $|$SHAP$|$.}
\label{fig:shap}
\end{figure}
...
\hline
\end{tabular}}
\caption{Enhanced tautology test results for E013. T1 evaluates correlation with three tautology proxies (volcano distance, road distance, nearest-site distance). T2 compares suitability predictions between surveyed ($\leq$5 km) and unsurveyed ($>$20 km) zones. T3 evaluates performance stability across road-distance quartiles as a proxy for survey intensity. T4 tests generalization to ``undiscovered'' sites using accessibility as a temporal proxy.}
\label{tab:tautology}
\end{table}
...
\textbf{Test 3 (Stratified CV by Survey Intensity):} This is strong anti-tautology evidence. Model performance across road-distance quartiles showed that Q4 (least surveyed; road\_dist $> 499$ m) achieved AUC = 0.788, \textit{exceeding} Q1 (most surveyed; road\_dist $\leq 43$ m) at AUC = 0.731 ($\Delta = +0.057$). If the model were merely learning survey visibility, performance should degrade in poorly-surveyed areas; instead, it improves.

\textbf{Test 4 (Temporal Split Validation):} We conducted a temporal stress test by training on easy-access sites (road distance $\leq$1 km, proxy for early discovery) and testing on hard-access sites (road distance $>$1 km, proxy for later discovery). The model achieved AUC = 0.755 on the held-out hard-access sites, only 0.030 below the spatial CV baseline (AUC = 0.785). This demonstrates that the model generalizes to sites that are harder to discover, providing strong evidence that it learns environmental suitability rather than historical survey patterns.

Overall, the enhanced tautology test suite returns a \textbf{CONDITIONAL PASS} verdict: the temporal validation (T4) and stratified CV (T3) provide robust evidence against tautology, while T1--T2 remain in the grey zone with metrics near their respective thresholds. We emphasize that tautology absence cannot be proven definitively from observational data alone; the temporal split (T4) provides the strongest available evidence by demonstrating generalization to sites the model was never trained on.
...
This supports the view that data-generation assumptions dominate model behavior under biased observation regimes \cite{georganos2021}.
...
Conceptualization, M.A.; methodology, M.A.; software, M.A.; validation, M.A.; formal analysis, M.A.; investigation, M.A.; resources, M.A.; data curation, M.A.; writing---original draft preparation, M.A.; writing---review and editing, M.A.; visualization, M.A.; project administration, M.A.
...
\textbf{Archaeological site data:} The compiled East Java archaeological site dataset (666 sites) is available in the repository at \texttt{data/processed/east\_java\_sites.geojson}. Sources include OpenStreetMap (ODbL license), Wikidata (CC BY-SA), Wikipedia Indonesia, and BPCB Jawa Timur publications.

\textbf{Environmental covariates:} The Copernicus GLO-30 DEM and derived layers (slope, aspect, TWI, TRI, river distance, road distance) are available from the Copernicus Programme (ESA) at \url{https://dataspace.copernicus.eu/} (free and open license). OpenStreetMap road and river data are available from \url{https://openstreetmap.org} (ODbL license).
...
\textbf{Code and software:} All source code, including the E013 hybrid model implementation, temporal validation (E014), and robustness checks, is available at GitHub: \url{https://github.com/neimasilk/volcarch-repo} (MIT license).
