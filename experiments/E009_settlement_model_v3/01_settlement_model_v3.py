"""
E009: Settlement Suitability Model v3 — adds SoilGrids clay/silt features.

Builds on E008 (AUC=0.695, BELOW MVR). Primary additions:
- clay: SoilGrids clay fraction (0-5cm mean)
- silt: SoilGrids silt fraction (0-5cm mean)

Run from repo root:
    py experiments/E009_settlement_model_v3/00_prepare_soilgrids.py
    py experiments/E009_settlement_model_v3/01_settlement_model_v3.py
"""
...
# Feature columns — E009 adds clay/silt on top of E008 feature set.
FEAT_COLS = ["elevation", "slope", "twi", "tri", "aspect", "river_dist", "clay", "silt"]
...
def main():
    ...
    print("=" * 60)
    print("E009: Settlement Suitability Model v3 (+ clay + silt)")
    print("=" * 60)
    ...
    raster_files = {
        ...
        "clay":       DEM_DIR / "jatim_clay.tif",
        "silt":       DEM_DIR / "jatim_silt.tif",
    }
    for name, path in raster_files.items():
        if not path.exists():
            if name in {"clay", "silt"}:
                print(f"ERROR: {path.name} missing. Run:")
                print("  py experiments/E009_settlement_model_v3/00_prepare_soilgrids.py")
            elif name == "river_dist":
                print(f"ERROR: {path.name} missing. Run:")
                print("  py tools/compute_river_distance.py")
            else:
    ...
    # Summarise key new features at known sites (sanity check)
    ...
    print(f"  Clay at sites: mean={site_feats['clay'].mean():.1f} "
          f"median={site_feats['clay'].median():.1f}")
    print(f"  Silt at sites: mean={site_feats['silt'].mean():.1f} "
          f"median={site_feats['silt'].median():.1f}")
    ...
    if best_auc >= 0.85:
        ...
        verdict = "REVISIT (0.65-0.75) — try TGB pseudo-absences next"
    else:
    ...
    progression = """
AUC progression:
  E007 (5 features): 0.659
  E008 (6 features): 0.695
  E009 (8 features): {xgb_mean_auc:.3f}
...
""".format(
    ...
    results_txt = f"""E009 — Settlement Suitability Model v3 Results
===============================================
Date: 2026-02-24
Features: {FEAT_COLS}
...
"""
    ...
    fig.suptitle(
        f"E009: Settlement Suitability Model v3 — Spatial CV Results\n"
        f"XGB AUC={xgb_mean_auc:.3f} ({auc_delta_e007:+.3f} vs E007, {auc_delta_e008:+.3f} vs E008)  |  {verdict}",
        fontsize=11
    )
