...
%   [FIGURES] All four required figures now exist (built 2026-08-05 from raw
%             per-run files by build_v02_figures.py): fig14 artefact two-panel,
%             fig15 dose-response, fig16 robust/contingent map, fig17 seed
%             stabilisation; fig10 study area redrawn with the canonical 13
%             centres (INT-1) and fig3 restated as the ladder under examination.
%             No \figtodo remains. \figtodo macro kept (unused) for safety.
%   [CITES]   ENM citations resolved 2026-08-05: banks2006, franklin2009,
%             howey2016, noviello2018, yaworsky2020 verified against publisher
%             records and added to references.bib. No [NEEDS CITATION] remains.
...
The transfer of species-distribution modelling into archaeology is likewise established --- from eco-cultural niche modelling \citep{banks2006}, through MaxEnt-based prediction of archaeological site distributions \citep{yaworsky2020, howey2016, noviello2018}, to the standard methodological reference \citep{franklin2009}.
...
This home-court inflation is specific to the hybrid design: the TGB design scored \emph{higher} on the uniform background than on its own (mean $-0.0054$, 22/60 cells positive).
...
That table scored the E013 model on its own (hybrid) background but scored the null models on a regular grid of all cells; given the inflation above, the reported $+0.122$ margin over the DKNS null mixed the two backgrounds.
Held to a common (uniform) background --- the sampling on which the nulls were scored --- the E013 design family reaches $0.706$ (XGBoost, the primary learner; E218), which still exceeds the spatial-interpolation null ($0.646$), but the margin shrinks from $+0.122$ to roughly $+0.06$, and the reported worst-seed margin ($+0.083$) moves toward the $+0.03$ detection floor of \S\ref{sec:ci}.
...
All results were independently verified by the authors, and every headline number in this revision was re-derived from the raw per-run outputs before submission, or from the stored experiment results where no per-run file exists.
