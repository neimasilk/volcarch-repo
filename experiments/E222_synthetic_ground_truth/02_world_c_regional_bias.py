"""
E222 World C: regional survey bias — does the hybrid quota work in its home regime?

Worlds A/B applied only road-distance survey bias. The E013 hybrid design's regional quota was built
for a different bias: REGIONALLY uneven survey intensity (some regencies far better surveyed than
others). Convicting the quota under road-only bias would be trying it outside its home regime -- a
straw man a hostile reviewer would name immediately.

World C: intensity = surface A (unchanged); survey acceptance = road-decay x region factor
[1.0, 0.4, 0.15, 0.05] across the frame quadrants, so the observed record concentrates in region 0
through survey effort, not through settlement alone.

Pre-registered fork (locked before running):
  - If hybrid(hf=0.0) -- the quota design -- beats random on truth-anchored recovery (auc_true,
    map_jaccard) in >= 60% of the 30 world x algorithm cases: the quota's rationale is VALIDATED in
    its home regime; the manuscript's message becomes "each design has a regime where its rationale
    holds, and the reported AUC cannot tell you which regime you are in".
  - If not: the quota has no demonstrated regime in our synthetic suite; the manuscript says so.

Run from repo root (~20 min):
    py experiments/E222_synthetic_ground_truth/02_world_c_regional_bias.py
"""
...
def main():
    ...
    print("=" * 74)
    print("E222 World C: regional survey bias — the quota's home regime")
    print("=" * 74)
    ...
    for w in range(N_WORLDS):
        wrng = np.random.default_rng(20_000 + 997 * w)
    ...
    df.to_csv(RESULTS_DIR / "e222c_runs.csv", index=False)
    ...
    out = {
        "experiment": "E222 World C", "n_worlds": N_WORLDS,
    ...
    out["quota_validated_in_home_regime"] = bool(
        out["quota_vs_random_auc_true"]["frac_positive"] >= 0.6
        or out["quota_vs_random_map_jaccard"]["frac_positive"] >= 0.6)
    with open(RESULTS_DIR / "e222c_outcome.json", "w", encoding="utf-8") as f:
    ...
    print(f"  QUOTA VALIDATED IN HOME REGIME     : {out['quota_validated_in_home_regime']}")
